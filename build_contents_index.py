from core.utils.db_utils import get_cursor_from_path, execute_sql_long_time_limitation
import json
import os, shutil
...
def build_content_index(db_path, index_path):
    ...
    cmd = "python -m pyserini.index.lucene --collection JsonCollection --input ./data/temp_db_index --index {} --generator DefaultLuceneDocumentGenerator --threads 16 --storePositions --storeDocvectors --storeRaw".format(
        index_path)
...
if __name__ == "__main__":
    ...
    print("build content index for spider-test's databases...")
    remove_contents_of_a_folder("./data/sft_data_collections/spider-test/db_contents_index")
    # build content index for spider's databases
    for db_id in os.listdir("./data/sft_data_collections/spider-test/test_database"):
        print(db_id)
        build_content_index(
            os.path.join("./data/sft_data_collections/spider-test/test_database/", db_id, db_id + ".sqlite"),
            os.path.join("./data/sft_data_collections/spider-test/db_contents_index/", db_id)
        )
